...
<!--&lt;!&ndash;            //该div是做分页的&ndash;&gt;-->
<!--            <div class="block">-->
<!--&lt;!&ndash;              @size-change="handleSizeChange"&ndash;&gt;-->
<!--&lt;!&ndash;              :current-page="current_page"&ndash;&gt;-->
<!--              <el-pagination-->
  ...
  export default {
    ...
    methods:{
      ...
      imtScreen(currentPage){
        ...
        this.$axios.get(`/findvalueLike/${keywords}/${this.pageSize}/${currentPage}?startTime=${stime}&endTime=${etime}`).then(resp => {
          if (resp && resp.data.code === 200) {
            this.articles = resp.data.result.articlesList
            this.articles=this.articles.sort(sortData)
      ...
      loadArticles () {
        ...
        this.$axios.get('/findvalueLike/' + keywords ).then(resp => {
          if (resp && resp.data.code === 200) {
