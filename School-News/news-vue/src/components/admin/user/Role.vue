...
<!--          </el-checkbox-group>-->
<!--        </el-form-item>-->
        <el-form-item label="功能菜单配置" label-width="120px" prop="menus">
          <el-tree
            :data="menus"
            :props="props"
            show-checkbox
...
<script>
  ...
  export default {
      ...
      deleteRole(id){
        this.$confirm('此操作将永久删除该角色, 是否继续?', '提示', {
          ...
          type: 'warning'
            ...
            this.$axios
              .post('/admin/deleteRoles/' + id).then(resp => {
              if (resp && resp.data.code === 200) {
                this.listRoles ()
              }
            })
    ...
    editRole (role) {
      ...
      // this.setHalfCheckedNodes(role.menus[i].id)
      for (let i = 0; i < role.menus.length; i++) {

        // console.log("role.menus[i].children.length",role.menus[i].children.length)
        // console.log("this.menus[i].children.length",this.menus[i].children.length)
        menuIds.push(role.menus[i].id)
        // console.log('r:'+role.id+'mid:'+ role.menus[i].id)
        for (let j = 0; j < role.menus[i].children.length; j++) {

          // console.log("childrean",role.menus[i].children[j])
          // console.log("j",j)
          menuIds.push(role.menus[i].children[j].id)
          console.log('r:'+role.id+'mid:'+ role.menus[i].children[j].id)

        }
      }
      this.selectedMenusIds = menuIds
      // console.log('r:'+role.id+'SS:'+ menuIds)
