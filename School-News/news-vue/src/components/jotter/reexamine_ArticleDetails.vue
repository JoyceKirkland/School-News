<template>
  <div class="articles-area">
    ...
    <el-card style="text-align: left;width: 990px;margin: 35px auto 0 auto">
      <div>
        <el-form v-model="article" label-width="100px" label-position="left" size="mini">
          <el-form-item label="审核意见" prop="desc">
            ...
            <el-input type="textarea" v-model="article.articleAuditMind" autosize
                      :disabled="article.audit_status.id==2||article.audit_status.id==7 ? false : true"></el-input>
          </el-form-item>
        </el-form>
      </div>
    </el-card>
    <div style="margin-left:-800px;margin-top:40px;margin-bottom:30px">
      <el-button style="left: 10px" type="primary" v-if="article.audit_status.id == 2 || article.audit_status.id == 7"
                 @click="saveAuditArticles(3,'复审通过')">复审通过</el-button>
      <el-button style="left: 100px;" type="danger" v-if="article.audit_status.id == 2 || article.audit_status.id == 7"
                 @click="saveAuditArticles(7,'不通过')">不通过</el-button>
    </div>
